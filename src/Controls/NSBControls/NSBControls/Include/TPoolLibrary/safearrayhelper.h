...
public:
    ...
    HRESULT MakeValues() {
        HRESULT hr;
        if (Dims() > 1) return E_NOTIMPL;
        if (VT != VT_VARIANT) return E_UNEXPECTED;
        if (pv) {
            hr = UnaccessData();
            if (FAILED(hr)) return hr;
        }
        LONG lSize = UBound() - LBound() + 1;
        if (!AccessData()) return E_OUTOFMEMORY;
        
        VARIANT*    pvv = (VARIANT*)pv;
        VARIANT     v;
        ::VariantInit(&v);
        LONG i;
        for (i = 0; i < lSize; i++) {
            ::VariantClear(&v);
            hr = VariantResolveToValue(&v,&pvv[i]);
            if (FAILED(hr)) {
                UnaccessData();
                return hr;
            }
            ::VariantClear(&pvv[i]);
            ::VariantCopyInd(&pvv[i],&v);
        }
        ::VariantClear(&v);
        hr = UnaccessData();
        return hr;
