...
public:
    ...
    LPTSTR  GetStringProperty(LPCTSTR szMethod,LPTSTR szResult,UINT uBufLen,UINT uCP = CP_ACP,HRESULT* phr = NULL,EXCEPINFO* pExcep = NULL) {
        ...
        ::SysFreeString(bstr);
        if (sz) {
            LONG lLen = min((LONG)_tcslen(sz),(LONG)(uBufLen - 1));
...
HRESULT ReprotCOMErrorInfo(LPCOLESTR wszDesc,REFIID riid = GUID_NULL,LPCOLESTR wszSource = NULL,HRESULT rhr = DISP_E_EXCEPTION);
HRESULT ReprotCOMErrorInfo(HINSTANCE hRes,UINT uResID,REFIID riid = GUID_NULL,LPCOLESTR wszSource = NULL,HRESULT rhr = DISP_E_EXCEPTION);
