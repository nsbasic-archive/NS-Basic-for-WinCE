#include <TPoolLibrary/automationhlp.h>
    ...
    #include "automationhlp-typepackers.cpp"
...
HRESULT ReprotCOMErrorInfo(LPCOLESTR wszDesc,REFIID riid,LPCOLESTR wszSource,HRESULT rhr) {
    HRESULT				hr;
	...
	if (FAILED(hr)) goto _REI_Err_Exit0;
	if (wszDesc) hr = pcei->SetDescription((LPOLESTR)wszDesc);
	if (FAILED(hr)) goto _REI_Err_Exit0;
	if (wszSource) hr = pcei->SetSource((LPOLESTR)wszSource);
	if (FAILED(hr)) goto _REI_Err_Exit0;
...
HRESULT ReprotCOMErrorInfo( HINSTANCE           hRes,
                            UINT                uResID,
                            REFIID              riid,
                            LPCOLESTR           wszSource,
                            HRESULT             rhr) {
	...
	// Transfer the information
	hr = pcei->SetGUID(riid);
	if (FAILED(hr)) goto _REI_Err_Exit1;
    TCHAR   _l[512];
    if (::LoadString(hRes,uResID,_l,512)) {
        _l[511] = '\0';
        #ifndef _UNICODE
            WCHAR   _s[512];
            ::MultiByteToWideChar(CP_ACP,0,_l,-1,_s,512);
            hr = pcei->SetDescription((LPOLESTR)_s);
        #else
            hr = pcei->SetDescription((LPOLESTR)_l);
        #endif
        if (FAILED(hr)) goto _REI_Err_Exit1;
    }
	if (wszSource) hr = pcei->SetSource((LPOLESTR)wszSource);
