...
END_COM_MAP()
	...
	HRESULT _StartAsyncDownload(BSTR bstrURL, IUnknown* pUnkContainer, long lRelative,LPCWSTR wszBaseURL,LPWSTR* pwszFullURL = NULL) {
		m_dwTotalRead = 0;
		...
		if (spBindHost == NULL) {
            ...
            if (lRelative && wszBaseURL) {
                ...
                // If it is requested output the full URL
                // However this is not considered crucial and out of memory error is just ignored
                if (pwszFullURL && pwsz) {
                    *pwszFullURL = new WCHAR[n];
                    if (*pwszFullURL) {
                        CopyMemory(*pwszFullURL,pwsz,n * sizeof(WCHAR));
                    }
                }
                if (lRelative == -2) {
                    if (pwsz) delete [] pwsz;
                    return S_OK;  // Asked for URL Calc only
                }
                hr = CreateURLMoniker(NULL, pwsz, &m_spMoniker);
                if (pwsz) delete [] pwsz;
            } else {
                if (lRelative == -2) return S_OK;  // Asked for URL Calc only
            ...
            if (lRelative == -2) return S_OK;  // Asked for URL Calc only - nothing to do when we have a host
        ...
        if (grfBindInfoF & BINDINFOF_URLENCODESTGMEDDATA) {
            ...
            *pdwSize = UBound-1;
            ...
            if (!FAILED(hr)) {
                ...
                hr = pmed->pstm->Write(pv,UBound-1,&cbWritten);
                delete [] ((CHAR*)pv);
                if (FAILED(hr)) {
                    pmed->pstm->Release();
                    pmed->pstm = NULL;
	...
	HRESULT StartAsyncDownload( T*                      pT,
        ...
        if (wszBaseURL && wszBaseURL[0]) {
            hr = _StartAsyncDownload(bstrURL, pUnkContainer, lRelative, wszBaseURL,pwszFullURL);
        } else {
	...
	static HRESULT Download(    T*                      pT, 
		...
		return pbsc->StartAsyncDownload(pT, pFunc, pProgress, bstrURL, NULL, pUnkContainer, lRelative,NULL,FALSE,CP_ACP,punkHold,dwID,nBindf,NULL,wszBaseURL,pwszFullURL);
	}
    ...
    static HRESULT PostForm(    T*                      pT,
        ...
        CComVariant     v;
        v = bstrData;
        return pbsc->StartAsyncDownload(pT, pFunc, pProgress, bstrURL, L"POST", pUnkContainer, lRelative,&v,TRUE,CP_ACP,punkHold,dwID,nBindFlags,NULL,wszBaseURL);
    }
    static HRESULT CustomOperation( T*                      pT,
		...
		return pbsc->StartAsyncDownload(pT,pFunc,pProgress,bstrURL,bstrVerb,pUnkContainer,lRelative,&vData,bFormPost,uCP,punkHold,dwID,nBindFlags,pFuncPermit,wszBaseURL);
    }
