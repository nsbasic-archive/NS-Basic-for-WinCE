...
Private Sub cmdNewOpen_Click()
   ...
   m_sFileName = lvwNew.SelectedItem.Tag
   m_bCancel = False
   Form_QueryUnload 0, 0
End Sub
...
'      itmX.SubItems(2) = m_dExistDate(i)
   ...
   End If
   lblSplash.Caption = gStringTable(1401)
End Sub
...
'   If (Right$(m_sFileName, 1) <> "\") Then m_sFileName = m_sFileName & "\"
   If (Mid(m_sFileName, Len(m_sFileName)) <> "\") Then m_sFileName = m_sFileName & "\"
...
Public Property Let NewImageList(ByRef ilsThis As MSComctlLib.ImageList)
   Set m_ilsNew = ilsThis
End Property

Public Property Let ExistingImageList(ByRef ilsThis As MSComctlLib.ImageList)
